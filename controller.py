...
class MainWindow_controller(QtWidgets.QMainWindow):
    ...
    def the_first_use(self):
        if os.path.isfile(self.user_data+'data.json'):
            time=(datetime.date.today()-datetime.timedelta(days=1)).strftime("%Y_%m_%d.json")
    ...
    def get_user_json(self):
        try:
            ...
            #for i in range(len(data_list)):
                ...
                #print(datetime.datetime.strptime(data_list[i].rsplit('\\',1)[1].replace('.json',''), "%Y_%m_%d"))
        except:
            pass
        try:
    ...
    def update(self):
        ...
        if (new_version>2.0):
            ...
            msgBox.setText("{} <br><a href='https://drive.google.com/file/d/1ujpmREZXaNJy_QcZQgBllefSx_wG3HiQ/view?usp=share_link'>按我跳轉至下載頁面\n</a>.".format(version_info))
