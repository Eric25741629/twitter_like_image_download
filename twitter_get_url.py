...
class download_thread(QThread, twitter_url.twitter_link_regex):
    ...
    def get_user_info(self, username, count=None):
    ...
    def first_get_url(self, response, get_twitter_id=None):
        ...
        get_media_url = list(dict.fromkeys(
            self.get_pic_link.findall(response.text)))  # 將所有圖片url使用正則表達法提取出來
        keep, media_url = self.add_url(
            media_url, get_media_url)  # 透過addurl將提取出來的圖片連結變成一個陣列
        new_cursor = response.json()['data']['user']['result']['timeline_v2']['timeline']['instructions'][
            0]['entries'][-1]['content']['value'].replace('+', "%2B").replace('/', "%2F").replace('=', '')
    ...
    def get_url_and_cursor(self, user_id, new_cursor, media_url):
        response = requests.get(twitter_url.like_url.format(
            user_id, new_cursor), headers=self.headers)
        get_media_url = list(dict.fromkeys(
    ...
    def new_func(self):
        ...
        self.get_token()
        if self.last_url == '':
            user_id, like_count = self.get_user_info(
                username=self.user_name.replace("@", ''), count=1)
        else:
            user_id = self.get_user_info(
                username=self.user_name.replace("@", ''))  # 獲得userid
            self.last_url = re.search("https://pbs.twimg.com/media/(.*)(?:.*|\?.*)", self.last_url.replace(
        ...
        if (not self.error_test(response)):
            return
        keep, media_url, new_cursor, twitter_id = self.first_get_url(
            response, get_twitter_id='YES')
        while (keep):
            '''
            因為無法確定何時會停止 所以這邊使用while迴圈
            當要偵測到已下載的時候 keep會變成0 終止無限迴圈
            '''
            keep, p_cursor, new_cursor, media_url = self.get_url_and_cursor(
                user_id, new_cursor, media_url)
    ...
    def download_twitter_img(self, filepath, link):
        try:
            name = link.replace(
                "https://pbs.twimg.com/media/", "").split(".")[0]
